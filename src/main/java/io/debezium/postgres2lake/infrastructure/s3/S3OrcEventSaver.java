...
public class S3OrcEventSaver extends AbstractEventSaver<OrcOpenedWriter> {
    ...
    protected OrcOpenedWriter createWriter(EventRecord event) {
        logger.infof("CONVERT AVRO SCHEMA TO ORC. INITIAL AVRO SCHEMA: %s", event.value().getSchema());
    ...
    private void saveRecord(GenericRecord record, TypeDescription schema, int rowIdx, VectorizedRowBatch vector) {
        ...
        for (var fieldIdx = 0; fieldIdx < orcFields.size(); fieldIdx++) {
            ...
            logger.infof("ORC field: %s, avro schema: %s, avro value: %s", orcField.toJson(), record.getSchema().getField(orcField.getFullFieldName()).schema(), avroFieldValue);
    ...
    private TypeDescription avroLogicalToOrcSchema(String connectLogicalType, Schema schema) {
        return switch (connectLogicalType) {
            ...
            case "io.debezium.time.ZonedTimestamp", "io.debezium.time.IsoTimestamp" -> TypeDescription.createTimestamp(); // initial type: string
            case "io.debezium.time.ZonedTime", "io.debezium.time.IsoTime", "io.debezium.time.Interval" -> TypeDescription.createLong(); // initial type: string
            ...
            case "io.debezium.time.Time", "io.debezium.time.MicroTime" -> TypeDescription.createLong(); // // initial type: int/long
            ...
            case "io.debezium.time.IsoDate" -> TypeDescription.createDate(); // initial type: string
            case "io.debezium.time.NanoTime" -> TypeDescription.createLong(); // initial type: long
            case "io.debezium.data.VariableScaleDecimal" -> TypeDescription.createString(); // unknown precision -> save as string
