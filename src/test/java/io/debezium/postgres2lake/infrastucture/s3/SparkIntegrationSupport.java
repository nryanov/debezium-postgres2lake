 ...
 */
public final class SparkIntegrationSupport {

    private static final AtomicReference<SparkSession> SESSION = new AtomicReference<>();

    private SparkIntegrationSupport() {
    }
    ...
    public static SparkSession session() {
        return SESSION.updateAndGet(s -> {
            ...
            var jdbcUrl = PostgresMinioTestResource.getJdbcUrl();
            var minioEndpoint = PostgresMinioTestResource.getMinioEndpoint();
    ...
    public static long countFileRowsWithPk(String format, String path, long primaryKey) {
        var df = session().read().format(format).load(path);
        return df.filter(col("primary_key").equalTo(lit(primaryKey))).count();
    }

    public static long countIcebergRowsWithPk(long primaryKey) {
        var spark = session();
        return spark.sql(
    ...
    public static long countPaimonRowsWithPk(long primaryKey) {
        var spark = session();
        return spark.sql(
    ...
    public static String fileDatasetBasePath() {
        return "s3a://warehouse/default/public/data";
    }
}
